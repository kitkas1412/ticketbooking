...
public class EventServiceImpl implements  EventService{
    ...
    public EventResponse createEvent(CreateEventRequest request){
        ...
        List<Ticket> tickets = IntStream.rangeClosed(1, request.totalTickets())
                .mapToObj(seatCode -> buildTicket(event, seatCode))
                .toList();
    ...
    private Ticket buildTicket(Event event, int seatCode) {
        return Ticket.builder().event(event).seatCode(seatCode).build();
    }
}
